...
* NoSQL 데이터베이스의 네 가지 유형  
    * 키-값 데이터베이스  
        ...
        |---|---|
        |guest1.name|james|
        |guest1.phoneNumber|12-34-56|
        |guest1.custType|Royal|

        **키<sub>key</sub>**는 고유해야한다. 서로 다른 데이터베이스에 동일한 이름의 키가 있는 것은 문제가 되지 않는다. 각 데이터베이스는 데이터베이스만의 **이름공간<sub>namespace</sub>**을 가지며 키는 이름공간에서 고유해야한다. 이름공간이란 식별자들을 모아놓은 집합이다.  
        보통 데이터베이스에서 이름공간을 한 개만 사용한다. 이 경우 데이터베이스에 있는 모든 키는 고유해야한다. 일부 키-값 데이터베이스는 하나의 데이터베이스에 이름공간을 여러 개 사용하기도 하는데 독립적인 식별자 집합을 여러 개 두도록 데이터베이스 구조를 만들어 사용하기 때문이다. 이러한 구조를 **버킷<sub>bucket</sub>**이라고 부르기도 한다.  
        **값<sub>value</sub>**은 키와 함께 저장되는 데이터를 말한다. 값은 다양한 데이터 타입으로 저장될 수 있으며 데이터 타입을 엄격하게 제한하지 않는다. 따라서 프로그램 설계에 따라 유효한 값의 범위를 결정해 필요에 따라 값을 선택할 수 있도록 제한하는 것은 전적으로 프로그램의 몫이다.  

        * 키-값과 관계형 데이터베이스의 차이점  
            키-값 데이터베이스는 테이블이 없기 때문에 테이블과 관련된 특성도 없다. 조인이 필요없으며 외부 키도 없다. SQL 질의 언어도 지원하지 않는다.  
            일부 키-값 데이터베이스는 하나의 데이터베이스에 독립된 이름공간 여러 개를 생성할 수 있는 버킷이나 컬렉션을 지원한다. 이들은 관계형 스키마와 유사한 것들을 구현하는데 사용될 수 있다.  
            키-이름 규칙과 관계형 데이터 모델에 있는 테이블과 관련 특성과 비슷한 점이 있는데, 테이블 이름은 키를, 테이블의 컬럼은 이름을 뜻한다. guest1.name와 같은 키-이름 규칙은 관계형 데이터 모델에 볼 수 있는 quest1 테이블과 name 컬럼의 테이블 구조를 참조한다.  
